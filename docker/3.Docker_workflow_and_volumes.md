...
You can declare volume to `docker run` with argument `-v {localFilePath}:{containerPath}`. You can also add some exclusions `-v {containerPathToExcludeFromVolume}`
...
ex: 
* Map workstation port 4000 -to-> 3000, **local** folder `/home/guillaume/dev/docker/section6` to **workstation** `/opt/app`, and use image _section_6_frontend_
  ```docker run -p 4000:3000 -v /home/guillaume/dev/docker/section6:/opt/app guihomediaz/section_6_frontend``` 
* Map workstation port 4000 -to-> 3000, **local** _current_ folder to **workstation** `/opt/app`, and use image _section_6_frontend_
  ```docker run -p 4000:3000 -v $(pwd):/opt/app guihomediaz/section_6_frontend``` 
* Map workstation port 4000 -to-> 3000, **local** _current_ folder to **workstation** `/opt/app`, and use image _section_6_frontend_ + use a particular container's folder as well
